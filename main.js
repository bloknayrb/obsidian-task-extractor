...
// src/types.ts
var DEFAULT_EXTRACTION_PROMPT = `You are a task extraction specialist. Extract actionable tasks from emails and meeting notes following these strict rules:
...
---END NOTE---`;
   ...
   */
  async processFileManually(file) {
    if (!file) {
      const message = "No active note to process";
      new import_obsidian2.Notice(`Task Extractor: ${message}`);
    ...
    try {
      ...
      new import_obsidian2.Notice("Extracting tasks from current note...");
      if (!this.settings.ownerName || this.settings.ownerName.trim().length === 0) {
...
var ExtractorSettingTab = class extends import_obsidian3.PluginSettingTab {
  ...
  addFrontmatterSection(containerEl) {
    ...
    new import_obsidian3.Setting(containerEl).setName("Reset to Default").setDesc("Restore the default task extraction prompt.").addButton((button) => button.setButtonText("Reset to Default").onClick(() => {
...
var TaskExtractorPlugin = class extends import_obsidian4.Plugin {
  ...
  async executeManualTaskExtraction() {
    const activeFile = this.app.workspace.getActiveFile();
    if (!activeFile) {
      new import_obsidian4.Notice("No active note to process");
      return;
    }
    if (activeFile.extension !== "md") {
      new import_obsidian4.Notice("Active file is not a markdown note");
      return;
    }
    try {
      ...
      new import_obsidian4.Notice("Error extracting tasks - see console");
    }
  }
};
